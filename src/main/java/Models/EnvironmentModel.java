    ...
    public void addUserProfile(UserProfileModel newUser) throws Exception {
        ...
        if (newUser.getName().equals("")
            ...
            try {
                ...
                UserProfileModel[] profileListAsArray = new UserProfileModel[this.userProfileModelList.size()];
                Console.print(
                        "New user '" + newUser.getName() + "'/" + newUser.getProfileType() + " has been created");

                CustomUserXStream uStream = new CustomUserXStream();
                uStream.toXML(this.userProfileModelList.toArray(profileListAsArray), new FileOutputStream("UserProfiles.xml"));
